...
def perceptron_margin(X, w_opt):
    ...
    #
    # Fill in the rest
    w_norm=torch.norm(w_opt,p=2)
    margin=torch.min(torch.abs(X @ w_opt)) / w_norm
    return margin
...
def knn_distance_matrix(test_x, train_x):
    ...
    """
    test_x_square=torch.sum(test_x ** 2,dim=1,keepdim=True)
    train_x_square=torch.sum(train_x ** 2,dim=1,keepdim=True)
    dot_product=test_x @ train_x.t()
    dist_mat=test_x_square-2.0*dot_product+train_x_square.t()
    return torch.sqrt(dist_mat)
